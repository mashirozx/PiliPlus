---
name: Sync Upstream
description: Check upstream/main and unpromoted main patches every 15 minutes, then promote a rebased main snapshot to release when needed.
on:
  schedule: every 15m
  workflow_dispatch:
    inputs:
...
steps:
  - name: Skip agent when release is current
    ...
    run: |
      ...
      git -C "$repository_dir" fetch --quiet --no-tags origin refs/heads/main:refs/remotes/origin/main
      ...
      unpromoted_main_commits="$(git -C "$repository_dir" cherry origin/release origin/main | grep '^+' || true)"
      release_only_main_commits="$(git -C "$repository_dir" cherry origin/main origin/release | awk '$1 == "+" {print $2}' | sort)"
      release_only_upstream_commits="$(git -C "$repository_dir" cherry upstream/main origin/release | awk '$1 == "+" {print $2}' | sort)"
      reverted_main_commits="$(comm -12 <(printf '%s\n' "$release_only_main_commits") <(printf '%s\n' "$release_only_upstream_commits"))"

      if [[ "$release_contains_upstream" == "true" && -z "$unpromoted_main_commits" && -z "$reverted_main_commits" ]]; then
        echo '{"type":"noop","message":"release contains the current upstream/main commit, all main patches, and no reverted main patches; skipped the agent."}' >> "$GH_AW_SAFE_OUTPUTS"
      else
        echo "release is missing upstream/main or main patches, or contains reverted main patches; continuing to the agent."
      fi
jobs:
  conclusion:
    pre-steps:
      - name: Fail incomplete upstream promotion
        ...
        run: |
          ...
          if [[ -n "$(git -C "$repository_dir" cherry origin/release origin/main | grep '^+' || true)" ]]; then
            echo "::error::release is missing one or more patch-equivalent commits from main; the rebase promotion failed or did not complete."
            exit 1
          fi
          release_only_main_commits="$(git -C "$repository_dir" cherry origin/main origin/release | awk '$1 == "+" {print $2}' | sort)"
          release_only_upstream_commits="$(git -C "$repository_dir" cherry upstream/main origin/release | awk '$1 == "+" {print $2}' | sort)"
          if [[ -n "$(comm -12 <(printf '%s\n' "$release_only_main_commits") <(printf '%s\n' "$release_only_upstream_commits"))" ]]; then
            echo "::error::release contains one or more fork patches reverted from main; the rebase promotion failed or did not complete."
            exit 1
          fi

          echo "release contains the current upstream/main commit, all main patches, and no reverted main patches."
safe-outputs:
      ...
      steps:
        - name: Verify and synchronize upstream
          run: |
            ...
            release_contains_source=false
            if git show-ref --verify --quiet "refs/remotes/origin/${release_branch}" && [[ -z "$(git cherry "origin/${release_branch}" "origin/${source_branch}" | grep '^+' || true)" ]]; then
              release_contains_source=true
            fi
            release_contains_reverted_source=false
            if git show-ref --verify --quiet "refs/remotes/origin/${release_branch}"; then
              release_only_source_commits="$(git cherry "origin/${source_branch}" "origin/${release_branch}" | awk '$1 == "+" {print $2}' | sort)"
              release_only_upstream_commits="$(git cherry upstream/main "origin/${release_branch}" | awk '$1 == "+" {print $2}' | sort)"
              if [[ -z "$(comm -12 <(printf '%s\n' "$release_only_source_commits") <(printf '%s\n' "$release_only_upstream_commits"))" ]]; then
                release_contains_reverted_source=true
              fi
            fi
            if [[ "$release_contains_upstream" == "true" && "$release_contains_source" == "true" && "$release_contains_reverted_source" == "true" ]]; then
              if [[ "$GITHUB_EVENT_NAME" != "workflow_dispatch" ]] || [[ "$force_release_rebase" != "true" ]]; then
                echo "${release_branch} already contains upstream/main and all ${source_branch} patches with no reverted source patches; nothing to synchronize."
                exit 0
              fi
            ...
            audit_branch="rebase-${upstream_sha}"
...
1. For scheduled runs, use `origin/main` as the source. For manual runs, the
  `source_branch` input is the source. Fetch that branch, `origin/release` when
  it exists, and `upstream/main`.
  Determine whether upstream is already an ancestor of `origin/release`. Also run
  `git cherry origin/release origin/<source_branch>`: lines beginning with `+`
  are source-branch patches not yet represented in release, even if rebasing has
  changed their commit SHAs. Also compare `git cherry origin/<source_branch>
  origin/release` and `git cherry upstream/main origin/release`; the intersection
  of their `+` commit SHAs identifies release-only fork patches that have been
  reverted from the source branch.
2. Call the `noop` tool only when release contains both `upstream/main` and every
  source-branch patch, with no release-only patch reverted from the source.
  Otherwise, inspect the source branch's commits and its `AGENTS.md` plus
  `skills/workflows/upstream-rebase.md` guidance.
3. Only when the source branch can be rebased linearly, call `rebase_upstream` with
