...
steps:
  - name: Skip agent when release is current
    ...
    run: |
      ...
      if [[ "$release_contains_upstream" == "true" && -z "$unpromoted_main_commits" ]]; then
        echo '{"type":"noop","message":"release contains the current upstream/main commit and all main patches; skipped the agent."}' >> "$GH_AW_SAFE_OUTPUTS"
      else
        echo "release is missing upstream/main or main patches; continuing to the agent."
      fi
jobs:
  conclusion:
    pre-steps:
      - name: Fail incomplete upstream promotion
        ...
        run: |
          ...
          echo "release contains the current upstream/main commit and all main patches."
safe-outputs:
      ...
      steps:
        - name: Verify and synchronize upstream
          run: |
            ...
            if [[ "$release_contains_upstream" == "true" && "$release_contains_source" == "true" ]]; then
              if [[ "$GITHUB_EVENT_NAME" != "workflow_dispatch" ]] || [[ "$force_release_rebase" != "true" ]]; then
                echo "${release_branch} already contains upstream/main and all ${source_branch} patches; nothing to synchronize."
                exit 0
              fi
...
1. For scheduled runs, use `origin/main` as the source. For manual runs, the
  ...
  changed their commit SHAs.
2. Call the `noop` tool only when release contains both `upstream/main` and every
  source-branch patch. Otherwise, inspect the source branch's commits and its
  `AGENTS.md` plus `skills/workflows/upstream-rebase.md` guidance.
