---
name: Upstream Release Rebase
description: Check upstream/main hourly and promote a rebased main snapshot to release only after upstream advances.
on:
  schedule: every 1h
  workflow_dispatch:
    inputs:
...
network:
  allowed:
    ...
    - github
concurrency:
  group: upstream-rebase-${{ github.repository }}
...
safe-outputs:
  ...
  jobs:
    rebase-upstream:
      description: Rebase main onto verified upstream/main, automatically resolve conflicts in favor of upstream, document them in an issue, force-push release, then dispatch a tagged build. Never use this for a dry run.
        ...
        - name: Verify and synchronize upstream
          run: |
            ...
            if [[ "$release_after_sha" == "$release_before_sha" ]]; then
              echo "${release_branch} already matches the rebased ${source_branch}; skipping build."
              exit 0
            fi
            ...
            if [[ ! "$app_version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
              echo "Unable to parse a release version from pubspec.yaml."
              exit 1
            fi
            ...
            build_payload="$(jq -nc --arg ref "$release_tag" --arg tag "$release_tag" '{ref: $ref, inputs: {build_android: "true", build_ios: "true", build_mac: "true", build_win_x64: "true", build_linux_x64: "true", tag: $tag}}')"
...
# Hourly Upstream Release Rebase
...
1. For scheduled runs, use `origin/main` as the source. For manual runs, the
  ...
  it exists, and `upstream/main`.
   Determine whether upstream is already an ancestor of `origin/release`.
2. If it is already present, call the `noop` tool with a short explanation. Do
   not promote ordinary changes on `main`; upstream must be newer than release.
3. If upstream is newer than release, inspect the source branch's commits and its
   `AGENTS.md` plus `skills/workflows/upstream-rebase.md` guidance.
4. Only when the source branch can be rebased linearly, call `rebase_upstream` with
...
5. If the rebase needs a conflict resolution or violates fork-specific guidance,
