...
3. Use the Android debug entry points from the repository root:
   ...
   The reusable container installs PowerShell from Microsoft's Ubuntu 24.04
   repository when needed. Debug install and hot-reload startup run
   `lib/scripts/patch.ps1 android` against `/cache/flutter` before building,
   matching the CI framework and `material_ui` patch sequence.
   The patch script honors `PUB_CACHE`; the Docker entry point supplies
   `/cache/pub` so `material_ui` is patched in the same persistent cache that
   Flutter uses for compilation.
   ...
   The Docker wrapper persists the Flutter 3.47.2 SDK, Pub packages, Gradle
   cache, and Android SDK under `build/docker-cache/`. Its Flutter SDK cache is
   version-specific, so an SDK update creates a clean worktree without modifying
   an older cached SDK. The first invocation populates the cache; later builds
   reuse it. Set `PILIPLUS_DOCKER_CACHE` to use a different cache location.
   Remove the reusable development container with `docker rm -f
   piliplus-android-dev` when its caches or Flutter version need a fresh process.
