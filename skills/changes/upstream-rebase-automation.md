...
- A synchronized release must contain both `upstream/main` and all patches from
  ...
  cause false promotions.
- Scheduled runs perform both checks with Git alone. Only when release contains
  upstream and `git cherry` produces no `+` main patches does the step write a
  GH-AW `noop` safe output and avoid AI-credit use. The 15-minute cadence
...
- The agent clones repositories only for read-only comparison. It requests the
  ...
  is absent from `release`. The safe-output job rebases `origin/main` onto
  verified `upstream/main` and force-pushes only `release`.
...
- The generated `conclusion` job independently fetches `release` and
  ...
  run fails when `release` lacks upstream or a patch from main, covering
  exhausted AI credits, agent no-ops, and any failed or incomplete promotion.
  Manual dry runs and `test_issue` credential checks are intentionally excluded.
