# Scheduled Upstream Rebase Automation
...
- Affected paths: `.gitattributes`, `.github/workflows/upstream-rebase.md`, and
  its generated `.github/workflows/upstream-rebase.lock.yml`.
- Upstream relationship: fork-only.

## Intent

Check the upstream project hourly and promote an upstream-rebased `main` snapshot
to this fork's `release` branch without exposing a repository write token to the
...
- `docker run --rm -v "$PWD:/workspace" -w /workspace alpine:3.21 ... gh-aw
  ...
  schedule-scattering warning when no Git remote is visible inside the
  container.
...
- `act workflow_dispatch -W .github/workflows/upstream-rebase.lock.yml --dryrun
