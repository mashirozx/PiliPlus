...
- Affected paths: `.gitattributes`, `.github/aw/actions-lock.json`,
  `.github/workflows/sync-main-to-release.yml`, `.github/workflows/sync-upstream.md`,
  and its generated `.github/workflows/sync-upstream.lock.yml`.
...
- A synchronized release is current when `upstream/main` is an ancestor of
  `origin/release`. Every scheduled check uses only this ancestry check, avoiding
  `git cherry` comparisons against rewritten `main` patches that can cause
  repeated promotions after a rebase.
- Scheduled runs perform this check with Git alone. When release contains the
  current upstream tip, the step writes a GH-AW `noop` safe output and avoids
  AI-credit use. When the tip is absent, the safe-output job rebases `main` onto
  the verified upstream commit and promotes the result to `release`. The 15-minute cadence
...
- The agent clones repositories only for read-only comparison. It requests the
  `rebase-upstream` custom safe output only when upstream is absent from
  `release`. The safe-output job then rebases `origin/main` onto verified
  `upstream/main` and force-pushes only `release`.
- `.github/workflows/sync-main-to-release.yml` uses `FORK_PUSH_TOKEN` whenever a
  commit is pushed to `main`, force-updating `release` to that exact commit. It
  shares the `release-promotion-<repository>` concurrency group with the upstream
  workflow, so the two release writers cannot race. The token-authenticated push
  also triggers the downstream `Auto Release` workflow.
...
- If Git reports merge conflicts, the safe-output job captures the conflicted
  ...
  `zzz/rebase-<full-upstream-sha>` and uses `ISSUE_POST_TOKEN` to create an issue
...
- The generated `conclusion` job independently fetches `release` and
  ...
  when `release` lacks the upstream tip, covering exhausted AI credits, agent
  no-ops, and any failed or incomplete promotion. Manual dry runs and
...
- CI scenario procedure: do not fetch or compare upstream in a developer
  ...
  modify/modify-conflict paths. Run each fixture
...
- `docker run --rm -v "$PWD:/workspace" -v "$HOME/.config/gh:/root/.config/gh:ro"
  ...
  warnings after switching to upstream-only release detection. The generated
  schedule runs four staggered delivery attempts per hour.
- `docker run --rm -v "$PWD:/workspace:ro" -w /workspace ruby:3.4-alpine ruby
  -e 'require "yaml"; YAML.load_file(".github/workflows/sync-main-to-release.yml")'`
