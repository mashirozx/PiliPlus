...
- The floating and default Material 3 navigation bars use one shared
  ...
  transparent. Its shared `64x32` indicator uses a `15px` top offset, matching
  the selected icon's Material 3 position when labels are always visible; this
  same offset applies throughout the transition and after it settles.
...
- `tool/docker_flutter.sh flutter analyze lib/pages/main/view.dart` completed
  without issues after aligning the non-floating Material 3 indicator.
- A Genymotion Phone hot-reload session verified the non-floating Material 3
  bar on home and dynamics. Both settled states and the dynamics-to-home
  transition kept the shared indicator vertically centered on its icon.
