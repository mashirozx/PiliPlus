...
- It resolves the current release commit from `GITHUB_SHA`, because a newly
  created GitHub Release can be visible before its tag ref is fetchable. The
  previous release commit is resolved from its tag; both commits' Git trees are
  checked before comparison. The notes list only `+` commits from `git cherry
  <previous> <current>`, which uses patch IDs rather than commit SHA equality.
